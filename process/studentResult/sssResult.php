...
include "../config/configTrihs.php";
...
class PDF extends FPDF {
        ...
        global $admno;
         ...
         while ($row17 = mysqli_fetch_array($result17)) {
             ...
             while ($row18 = mysqli_fetch_array($result18)) {
                 $avg_current = $avg_current + (float)$row18['ca1'] + (float)$row18['ca2'] + (float)$row18['ca3'] + (float)$row18['exam'];
                 $counter++;
             }
    ...
    function header(){
        ...
        $this->cell(200,5,'THE REEDEMER'."'".'S INTERNATIONAL HIGH SCHOOL',0,0,'C');
        $this->Ln(1);
        /*$this->Image('sign/traco.png',10,10,10);*/
        $this->SetFont('Arial','',8);
        $this->cell(70,20,$this->Image('http://www.trihs.tracoportal.com/user/sign/trihs.png',10,10,22),0,0,'C');
        $this->cell(50,20,$this->Image('http://www.trihs.tracoportal.com/user/file/profile/'.$_SESSION['sd']['photo'],180,10,24),0,0,'C');
        $this->Ln(6);
    ...
    function footer(){
        $this->SetY(-15);
        $this->SetFont('Arial','',8);
        $this->cell(10,10,$this->Image('http://www.trihs.tracoportal.com/user/sign/sign-2.png',55,270,40),0,0,'C');
    ...
    function headerTable(){
        ...
        $this->cell(25,5,'1st Summary 10%',1,0,'C');
        $this->cell(25,5,'2nd Summary 20%',1,0,'C');
        $this->cell(17,5,'Project 10%',1,0,'C');
    ...
    function viewTable($conn){
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $this->cell(25,6,(float)$row['ca1'],1,0,'C');
            $this->cell(25,6,(float)$row['ca2'],1,0,'C');
            $this->cell(17,6,(float)$row['ca3'],1,0,'C');
...
$pdf->AliasNbPages();
$pdf->AddPage('P','A4',0);
$pdf->Image('http://www.trihs.tracoportal.com/user/sign/trihs_bg.png',10,10,189);
$pdf->headerTable();
$pdf->viewTable($conn);
$pdf->updateCard($conn);
$pdf->Output();
